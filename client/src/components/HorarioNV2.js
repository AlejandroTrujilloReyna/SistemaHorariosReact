...
const HorarioNV2 = () => {
  ...
  const columns = [
    ...
    {field: 'lunes', header: 'Lunes', filterHeader: 'Filtro'},
    {field: 'martes', header: 'Martes', filterHeader: 'Filtro'},
    {field: 'miercoles', header: 'Miercoles', filterHeader: 'Filtro'},
    {field: 'jueves', header: 'Jueves', filterHeader: 'Filtro'},
    {field: 'viernes', header: 'Viernes', filterHeader: 'Filtro'},
    {field: 'sabado', header: 'Sabado', filterHeader: 'Filtro'},
    {field: 'domingo', header: 'Domingo', filterHeader: 'Filtro'},         
  ];  
  ...
  const renderBody = (rowData, field) => {
    if (field === 'clave_Dia') {
      ...
      if (rowData.clave_Dia === 1) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'martes') {
      if (rowData.clave_Dia === 2) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'miercoles') {
      if (rowData.clave_Dia === 3) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'jueves') {
      if (rowData.clave_Dia === 4) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'viernes') {
      if (rowData.clave_Dia === 5) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'sabado') {
      if (rowData.clave_Dia === 6) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
    }
    else if (field === 'domingo') {
      if (rowData.clave_Dia === 7) {
        return `${rowData.hora_Entrada} - ${rowData.hora_Salida}`;
      } else {
        return ''; // Vacío si clave_Dia no es 1
      }
